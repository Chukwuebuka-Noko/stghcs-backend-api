...
class SupervisorGigController extends Controller
{
    ...
    public function index()
    {
        ...
        $gigs = Gig::with(['client','schedule','supervisor','assignments.assignee'])
            ...
            })->whereNotIn('status', ['ended', 'completed'])->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->filled('support_worker_id')) {
            $this->assignSupportWorkerToGig($gig, $support_worker);
        }
    ...
    protected function assignSupportWorkerToGig($gig, $support_worker)
{
    ...
    if ($this->hasScheduleConflict($user_id, $currentSchedule)) {
        return response()->json(['status' => 409, 'response' => 'Conflict', 'message' => 'This schedule conflicts with an active schedule.']);
    }
    ...
    protected function updateAssignedSupportWorkerToGig($gig, $support_worker)
{
    ...
    public function complete(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['status' => 200, 'response' => 'Gig Completed', 'message' => 'Gig and schedule has been completed successfully']);
        } catch (\Exception $e) {
            // Rollback the transaction in case of an error
            DB::rollback();
