...
class UserGigController extends Controller
{
    ...
    public function accept_gig(Request $request)
    {
        ...
        if ($user->hasRole('CSP')) {
            $gig = Gig::where('id', $request->gig_id)->with(['schedule'])->first();
        ...
        ActivityLog::create([
            ...
            'description' => auth()->user()->last_name . ' ' . auth()->user()->first_name . ' accepted gig with ID ' . $request->gig_id . ' at ' . Carbon::now()->format('h:i:s A'),
